...
## 1. NVIDIA api 키 발급후, 교수님의 코드를 runpod에서 돌려보기
...
peoplenet 모델 파일을 직접 코랩에 업로드하는 코드를 사용했었는데, 백업은 구글 드라이브와 현재 리포지터리에 해두었다.<br>

**앙상블(ensemble)이란, 여러 개의 서로 다른 모델을 함께 사용해서 단일 모델보다 더 견고하고 정확한 예측을 얻는 기법을 말합니다. 객체 검출(Object Detection)에 쓰임**<br>
장점
- 모델 간 강·약점을 보완
